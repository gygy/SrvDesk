...
internal sealed class HelpDetailPanel : Panel
{
    ...
    public void ShowPlaceholder(string? groupTitle = null)
    {
        ...
        BuildSections([
            ("操作", "开=采用「优化建议值」；关=恢复「系统默认值」。修改后点击底部「应用推荐」写入系统。「系统当前值」为读取到的本机实际状态。"),
            ("分类筛选", "命令栏「分类」可按 Server 推荐、优化推荐、已优化、未优化快速过滤当前页项目。"),
    ...
    public void ShowUsageGuide()
    {
        ...
        BuildSections([
            ("工作流程", "1. 选择左侧分类 → 2. 勾选优化建议或从「预设」菜单载入方案 → 3. 点击「应用推荐」写入系统。"),
            ("三列含义", "优化建议值=开关目标；系统默认值=出厂；系统当前值=本机实际（刷新/应用后更新）。"),
