...
internal sealed class HelpDetailPanel : BufferedPanel
{
    ...
    public void ShowEmbeddedGuide(string pageTitle)
    {
        ...
        if (pageTitle == "资源管理器")
        {
            _summary.Text = "资源管理器相关开关可即时写入；任务栏搜索/隐藏等改完后需点本页「应用到系统」（会重启资源管理器）。";
            BuildSections([
                ("任务栏", "搜索隐藏/显示、对齐、合并、自动隐藏等：先改开关，再点底部「应用到系统」才会写入并生效。"),
                ("其它项", "显示扩展名、隐藏文件等多数项切换后立即写入；若界面未变可再点「刷新」。"),
                ("与分组页", "同一设置若在「性能及安全」等分组中也有，本页用于逐项微调；分组页适合配合预设批量应用。"),
            ]);
            _footer.Text = "桌面维护等请从顶部「工具」菜单打开";
            return;
        }
