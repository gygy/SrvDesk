namespace WinOpt;

/// <summary>批量设置列表的列坐标（表头与行必须一致，避免开关被文字盖住）。</summary>
internal static class SettingListLayout
{
    public const int InfoX = 8;
    public const int ItemX = 28;
    /// <summary>「说明」列：对应哪里 · 何时建议。</summary>
    public const int NoteX = 248;
    public const int NoteW = 268;
    /// <summary>「优化建议值」表头起点（开关居中对齐其下）。</summary>
    public const int RecommendHeaderX = 528;
    public const int RecommendHeaderW = 90;
    public const int ToggleX = 546;
    public const int ToggleW = 52;
    /// <summary>系统默认值列。</summary>
    public const int SystemX = 648;
    public const int SystemW = 100;
    /// <summary>系统当前值列。</summary>
    public const int CurrentX = 760;
    public const int CurrentW = 110;
    /// <summary>项目文字右缘与说明列左缘的间隙。</summary>
    public const int TextNoteGap = 8;
}
