...
/// 基础 16px / Segoe UI（中文回退 YaHei UI）；勿对逻辑 pt 再乘 DPI（PerMonitorV2）。
/// </summary>
internal static class UiFit
{
    ...
    public static Font UiFontCode(float pt = 11.25f) =>
    ...
    /// <summary>舒适行高（14→22 / 15→24 / 16→24 / 18→28 / 22→32），不低于实测字形。</summary>
    ...
    public static int ComfortLineHeightDesignPx(float sizeInPoints)
    {
        ...
        if (px <= 16.5f) return 24;
        if (px <= 18.5f) return 28;
        if (px <= 22.5f) return 32;
        return 36;
    }
    ...
    /// </summary>
    public static int ControlHeight(Font? font = null, int designMin = 38) =>
