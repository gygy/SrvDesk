...
internal static class ServiceOptimizeCatalog
{
    ...
    [
        ...
        // —— 性能 ——
        E("SysMain", "SysMain / Superfetch（超级抓取）", "SysMain / Superfetch",
            "性能", "Performance", ServiceOsTarget.All,
            ServiceRecommend.Disable, ServiceRecommend.Disable,
            "【应禁用】预读占磁盘 I/O，强烈推荐关闭（原名 Superfetch，现名 SysMain）",
            "Disable: prefetch I/O cost; strongly recommended off (formerly Superfetch, now SysMain)."),
        ...
        E("WSearch", "Windows Search（索引服务）", "Windows Search (Indexing)",
            "性能", "Performance", ServiceOsTarget.All,
            ServiceRecommend.Disable, ServiceRecommend.Disable,
            "【应禁用】索引占 CPU/磁盘较大，强烈推荐关闭；文件搜索可用 Everything 代替",
            "Disable: indexing is heavy on CPU/disk; strongly recommended off—use Everything instead."),
        E("cisvc", "索引服务（旧版）", "Indexing Service (legacy)",
            "性能", "Performance", ServiceOsTarget.All,
            ServiceRecommend.Disable, ServiceRecommend.Disable,
            "【应禁用】旧版索引服务，现代系统通常已无；有则强烈推荐关闭",
            "Disable legacy Indexing Service if present; strongly recommended off."),
        ...
        E("RemoteRegistry", "远程注册表", "Remote Registry",
            ...
            ServiceRecommend.Disable, ServiceRecommend.Disable,
            "【应禁用】允许远程改注册表；强烈推荐关闭。",
            "Disable: remote registry access; strongly recommended off."),
        E("bthserv", "蓝牙支持服务", "Bluetooth Support Service",
            "硬件外设", "Hardware", ServiceOsTarget.Client,
            ServiceRecommend.Disable, ServiceRecommend.Disable,
            "【应禁用】不用蓝牙设备时强烈推荐关闭；不影响普通无线鼠标（多数走 2.4G 接收器）。",
            "Disable if unused; strongly recommended off. Most wireless mice use 2.4G dongles, not Bluetooth."),
        E("wuauserv", "Windows Update", "Windows Update",
            "更新", "Updates", ServiceOsTarget.All,
            ServiceRecommend.Disable, ServiceRecommend.Manual,
            "【应禁用】可禁用以避免不必要自动更新；需要打补丁时再开（风险较高，Server 建议手动）。",
            "Disable to avoid unwanted auto-updates; re-enable when patching (higher risk; Manual on Server)."),
