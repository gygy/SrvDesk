...
internal sealed class ServiceOptimizeDialog : Form, IEmbeddedSettingsPage
{
    ...
    private readonly ListViewStickySelection<ServiceOptimizeRow> _sticky;
    ...
    public ServiceOptimizeDialog()
    {
        ...
        _sticky = new ListViewStickySelection<ServiceOptimizeRow>(_list);
    ...
    {
        var sticky = _sticky.Get();
        if (sticky.Count == 0)
            return [];

        // 优先用当前列表里的同行对象（刷新后 Tag 已换新实例）
        var byName = new Dictionary<string, ServiceOptimizeRow>(StringComparer.OrdinalIgnoreCase);
        foreach (ListViewItem row in _list.Items)
        {
            if (row.Tag is ServiceOptimizeRow item)
                byName[item.ActualServiceName] = item;
        }

        var resolved = new List<ServiceOptimizeRow>();
        foreach (var s in sticky)
        {
            if (byName.TryGetValue(s.ActualServiceName, out var live))
                resolved.Add(live);
            else
                resolved.Add(s);
        }
